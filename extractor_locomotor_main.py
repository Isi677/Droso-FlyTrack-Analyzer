...
def main_backend(datos_grupos, excels_coordenadas, 
                 filtro_distancia, filtro_actividad_pausas, 
                 analizar_preferencia, n_rings, frames_totales, 
                 record_time, stop_event):
    tiempo = record_time / frames_totales
        
    for nombre_grupo in datos_grupos:
        #Se obtienen los videos de cada grupo
        info_videos = excels_coordenadas[nombre_grupo]
        listados_R = []
        listados_E = []
        listados_C = []
        listados_pared_R = []
        ...
        for video in info_videos:
            print(f"Empezando analisis de: {video[0]}...")
            ...
            if not stop_event.is_set():
                ...
                if not os.path.exists(f"./Analisis Datos Crudos/{nombre_grupo}"):
                    os.makedirs(f"./Analisis Datos Crudos/{nombre_grupo}")
                    
                print("Guardando datos crudos...")
                with pd.ExcelWriter(f"Analisis Datos Crudos/{nombre_grupo}/{nombre_video}.xlsx") as writer:
                ...
                if not os.path.exists(f"./Analisis Centrofobismo/{nombre_grupo}"):
                    os.makedirs(f"./Analisis Centrofobismo/{nombre_grupo}")
                with pd.ExcelWriter(f"Analisis Centrofobismo/{nombre_grupo}/{nombre_video}.xlsx") as writer:
                ...
                if analizar_preferencia == None:
                    ...
                    if not os.path.exists(f"./Analisis Preferencia/{nombre_grupo}"):
                        os.makedirs(f"./Analisis Preferencia/{nombre_grupo}")
        
                    with pd.ExcelWriter(f"Analisis Preferencia/{nombre_grupo}/{nombre_video}.xlsx") as writer:
        ...
        if not stop_event.is_set():
            ...
            summary_C = fa.summary(listados_C, n_moscas_video, 
                                  ...
                                  analizar_preferencia, listados_pref_C, n_rings, 
            ...
            summary_R = fa.summary(listados_R, n_moscas_video, 
                                  ...
                                  analizar_preferencia, listados_pref_R, n_rings, 
            ...
            writer = pd.ExcelWriter(f"Analisis Resumen/Summary - {nombre_grupo}.xlsx", engine='openpyxl')
